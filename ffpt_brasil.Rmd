...
```{r Ano de Implementacao, fig.align = 'center', out.width = "70%", dpi=300}
...
```{r Taxa de urbanização dos municípios boxplot, echo=FALSE, fig.align = 'center', out.width = "80%", dpi=300}
...
```{r Taxa de Urbanização densidade, echo=FALSE, fig.align = 'center', out.width = "80%", dpi=300}
...
banco_completo_5 %>% 
 ...
 # mutate(media_nacional = mean(value)) %>% 
  ...
  mutate(UF = case_when(UF == "Ce" ~ "CE", UF == "Mg" ~ "MG", UF == "Sp" ~"SP", 
                      ...
                      UF == "Rj" ~ "RJ", TRUE ~ UF)) %>% 
  ggplot() +
  geom_col(aes(x = reorder(nome_muni, value), y = value, fill = UF), width = 0.65,colour="white") +
#  geom_errorbar(mapping = aes(x = nome_muni, y = media_nacional, ymin = media_nacional, ymax = media_nacional), size = 0.5) +
  facet_wrap(~name) +
...
## 9. Índice de Vulnerabilidade Social de Infraestrutura Urbana

```{r infra urbana - gráfico, fig.align='center', out.width= "80%", dpi=300}

banco_completo_5 %>% 
  mutate(media_nacional = mean(ivs_infra_urbana)) %>% 
  filter(tarifa_zero == "sim") %>% 
  mutate_if(is.character, str_to_title) %>%
  mutate(UF = case_when(UF == "Ce" ~ "CE", UF == "Mg" ~ "MG", UF == "Sp" ~"SP", 
                        UF == "Pr" ~ "PR", UF == "Rs" ~"RS", UF == "Go" ~"GO", 
                      UF == "Rj" ~ "RJ", TRUE ~ UF)) %>% 
  ggplot() + 
  geom_col(aes(x = reorder(nome_muni, ivs_infra_urbana), y = ivs_infra_urbana, fill = UF), width = 0.65,colour="white") +
   geom_errorbar(mapping = aes(x = nome_muni, y = media_nacional, ymin = media_nacional, ymax = media_nacional), size = 0.5) +
  xlab("") +
  ylab("") +
  labs(title = "Infraestrutura Urbana", 
       subtitle = "Índice de vulnerabilidade social da infraestrutura urbana") +
  scale_fill_brewer(name = "UF", palette = "Set3")+
  theme_minimal() + 
    theme(plot.title = element_text(family = "serif", size = 12, face = "bold", hjust = 0.5, colour = "black"),
...
```{r Delocamento e Pobreza 1, fig.align = 'center', out.width = "80%", dpi=300}
...
```{r Delocamento e Pobreza 2, fig.align = 'center', out.width = "80%", dpi=300}
...
```{r Deslocamento e Pobreza boxplot, echo=FALSE, fig.align = 'center', out.width = "80%", dpi=300}
...
```{r Renda per capta, fig.align = 'center', out.width = "80%", dpi=300}
...
```{r grafico ivs, fig.align = 'center', out.width = "80%", dpi=300}
...
# tabela 
  ...
  tbl_2 %>% 
      ...
      `Muito Baixo` = colDef("Muito Baixa", minWidth = 75)), 
    columnGroups = list(
...
```{r Grafico IDHM, fig.align = 'center', out.width = "80%", dpi=300}
...
```{r mapa,fig.align = 'center', out.width = "80%", dpi=300}
