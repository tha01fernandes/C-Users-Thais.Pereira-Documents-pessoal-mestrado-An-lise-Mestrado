...
however, the northeast only has 3 municipalities with Tarifa Zero. 
...
# Tabela com a Região dos Municípios
  ...
  tally() %>% 
   ...
   # select(- n) %>% 
  # kable(digits=1, col.names=c("Region", "Number of municipalities with Tarifa Zero")) %>%
  ...
  reactable(
      ...
      n = colDef("Number of municipalities with Tarifa Zero", minWidth = 300, align = "center")))
...
# Tabela com a UF dos municípios 
  ...
  # mutate(pct = paste0(round(pct * 100, 0),"%")) %>% 
  # select(- n) %>% 
  # kable(digits=1, col.names=c("Federative Unit", "Municipalities with Tarifa Zero")) %>%
  # kable_classic_2(full_width = F, html_font = "Cambria")
    reactable(
      outlined = F, 
      fullWidth = F,
      ...
      align = "center"),
      columns = list(
      UF = colDef("Federative Unit", minWidth = 120, align = "center"),
      n = colDef("Number of municipalities with Tarifa Zero", minWidth = 300, align = "center")))
...
# Tabela com a classe populacional dos municípios 
  ...
  mutate(classe_pop = factor(classe_pop, levels = c("Up to 5.000", 
                                                    ...
                                                    "Greater than 500.000", ordered = T))) %>%
      reactable(
      outlined = F, 
      fullWidth = F,
      bordered = TRUE,
      ...
      align = "center"),
      columns = list(
      classe_pop = colDef("Population Class", minWidth = 190, align = "center"),
      n.x = colDef("Municipalities with Tarifa Zero", minWidth = 230, align = "center"), 
      n.y = colDef("Municipalities without Tarifa Zero", minWidth = 240, align = "center")))
...
media_pop_ffpt <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(median(pop_estimada_2019)) %>% 
  round(digits = 0) %>% 
  format_milhar()
...
media_pop_geral <- banco_completo_5 %>% 
  filter(tarifa_zero != "sim") %>% 
  summarize(median(pop_estimada_2019, na.rm = TRUE)) %>% 
  round(digits = 0) %>% 
  format_milhar()
...
In the graph below, we can see the estimated population of 2019 by the IBGE for all cities that adopted the Tarifa Zero, together with the national median (vertical line). The population median of the municipalities with the Zero Tariff is `r media_pop_ffpt` thousand inhabitants, with very different amounts, we have a maximum of `r max_pop_ffpt` as is the case of Caucaia in Ceará, and a minimum of `r min_pop_ffpt` inhabitants in Pedro Osório in Rio Grande do Sul (IBGE, 2019). In municipalities that have not adopted the policy, the population median is `r media_pop_geral`, but also with very different values, reaching a maximum of `r max_pop_geral` thousand inhabitants, as is the case of the city of São Paulo. In the chart below, it is possible to compare the municipalities with the Tarifa Zero against each other: 
...
options(scipen=999)
banco_completo_5 %>% 
  mutate(media_nacional = median(pop_estimada_2019)) %>% 
  filter(tarifa_zero == "sim") %>% 
  mutate_if(is.character, str_to_title) %>%
  mutate(UF = case_when(UF == "Ce" ~ "CE", UF == "Mg" ~ "MG", UF == "Sp" ~"SP", 
                        UF == "Pr" ~ "PR", UF == "Rs" ~"RS", UF == "Go" ~"GO", 
...
media_urbanizacao_fftp <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(median(taxa_urbanizacao)) %>% 
  round(digits = 2) %>% 
  pull()

media_urbanizacao_geral <- banco_completo_5 %>% 
  filter(tarifa_zero != "sim") %>% 
  summarize(median(taxa_urbanizacao, na.rm = TRUE)) %>% 
  round(digits = 2) %>% 
  pull()
...
banco_completo_5 %>% 
  mutate(media_nacional = median(taxa_urbanizacao)) %>% 
  filter(tarifa_zero == "sim") %>% 
  mutate_if(is.character, str_to_title) %>%
  mutate(UF = case_when(UF == "Ce" ~ "CE", UF == "Mg" ~ "MG", UF == "Sp" ~"SP", 
                        UF == "Pr" ~ "PR", UF == "Rs" ~"RS", UF == "Go" ~"GO", 
...
There is a difference between the median urbanization rate of municipalities
who adopted Tarifa Zero and those who did not. The median rate of the first group is
...
approximate the median of the second group, Itatiaiuçu (MG) with 62% of
...
banco_completo_5 %>% 
  mutate(media_nacional = median(densidade_demografica)) %>% 
...
2019. We ca see that just one city implemented Tarifa Zero in the same year of the plan's elaboration, Campo Belo in the state of Minas Gerais. 
furthermore, just Cerqilho and Vargem Grande Paulista adopted this policy after the plan's elaborations, the others cities did it before.  

```{r Plano de Mobilidade Tabela}

# Tabela plano de mobilidade urbana 2

ano_implem <- tibble(nome_muni=c("Campo Belo","Monte Carmelo", "Volta Redonda","Agudos", "Cerquilho", "Vargem Grande Paulista", "Ivaiporã"),
        ano_implem =c(2019, 1994, 2018, 2003, 2020, 2019, 2001))
...
tab_plano_mob <- banco_completo_5  %>% 
  ...
  mutate_if(is.character, str_to_title) %>% 
  left_join(ano_implem, by = "nome_muni")

tab_plano_mob %>% 
  mutate(UF = case_when(UF == "Mg" ~ "MG", UF == "Sp" ~"SP", UF == "Pr" ~ "PR", UF == "Rj" ~ "RJ", TRUE ~ UF)) %>% 
 reactable(
      outlined = F, 
      fullWidth = T,
      bordered = TRUE,
      ...
      align = "center"),
      columns = list(
      UF = colDef("Federative Unit", minWidth = 130, align = "center"),
      nome_muni = colDef("Municipality", minWidth = 210, align = "center"), 
      classe_pop = colDef("Populational Class", minWidth = 240, align = "center"),
      ano_elaboracao = colDef("Year of Elaboration", minWidth = 220, align = "center"),
      ano_implem = colDef("Implementation's year of Tarifa Zero", minWidth = 220, align = "center")))
...
media_indice_gini_tf <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(median(indice_gini)) %>% 
  round(digits = 2) %>% 
  pull()

media_indice_gini_geral <- banco_completo_5 %>% 
  filter(tarifa_zero != "sim") %>% 
  summarize(median(indice_gini, na.rm = TRUE)) %>% 
  round(digits = 2) %>% 
  pull()
...
media_indice_theil_tf <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(median(indice_theil)) %>% 
  round(digits = 2) %>% 
  pull()

media_indice_theil_geral <- banco_completo_5 %>% 
  filter(tarifa_zero != "sim") %>% 
  summarize(median(indice_theil, na.rm = TRUE)) %>% 
  round(digits = 2) %>% 
  pull()
...
banco_completo_5 %>% 
  ...
  group_by(name) %>% 
  mutate(media_nacional = median(value)) %>% 
...
In the density graph below it is possible to see that there are
...
media_pct_pobres_fftp <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(median(pct_pobres)) %>% 
  round(digits = 2) %>% 
  pull()

media_pct_pobres_geral <- banco_completo_5 %>% 
  filter(tarifa_zero != "sim") %>% 
  summarize(median(pct_pobres, na.rm = TRUE)) %>% 
  round(digits = 2) %>% 
  pull()

diferença <- media_pct_pobres_geral - media_pct_pobres_fftp 

```

## 9. Percentage of Poor

The graph below shows the percentage of poor people in each municipality studied and the vertical line represents the national median.
...
from Brazil, and was produced from 2010 census data. There is a considerable difference between the medians found in the
...
`r media_pct_pobres_geral`%, which corresponds to a difference of `r diferença`%. 


```{r Porcentagem de pobres, fig.align = 'center', out.width = "80%", dpi=300}

banco_completo_5 %>% 
  mutate(media_nacional = median(pct_pobres/100)) %>% 
...
media_deslocamento_fftp <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(median(taxa_pop_vulneravel_retorna_diariamente_trabalho*100)) %>% 
  round(digits = 2) %>% 
  pull()

media_desloc_geral <- banco_completo_5 %>% 
  filter(tarifa_zero != "sim") %>% 
  summarize(median(taxa_pop_vulneravel_retorna_diariamente_trabalho*100, na.rm = TRUE)) %>% 
  round(digits = 2) %>% 
  pull()
...
media_desloc_fftp <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(median(ocup_desl)) %>% 
  round(digits = 2) %>% 
  pull()

media_desloc_geral <- banco_completo_5 %>% 
  filter(tarifa_zero != "sim") %>% 
  summarize(median(ocup_desl, na.rm = TRUE)) %>% 
  round(digits = 2) %>% 
  pull()
...
media_desloc2_fftp <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(median(taxa_pop_vulneravel_retorna_diariamente_trabalho)) %>% 
  round(digits = 2) %>% 
  pull()

media_desloc2_geral <- banco_completo_5 %>% 
  filter(tarifa_zero != "sim") %>% 
  summarize(median(taxa_pop_vulneravel_retorna_diariamente_trabalho, na.rm = TRUE)) %>% 
  round(digits = 2) %>% 
  pull()
...
media_pib_1 <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(median(Pib_Municipal_2018)) %>%
  round(digits = 0) %>% 
  pull()

media_pib_2 <- banco_completo_5 %>% 
  filter(tarifa_zero != "sim") %>% 
  summarize(median(Pib_Municipal_2018, na.rm = TRUE)) %>% 
  round(digits = 0) %>% 
  pull()
...
GDP per capita is data produced by IBGE from 2018. The median GDP per capita of the municipalities studied is `r media_pib_1` while the median in Brazilian municipalities that have not adopted the policy is `r media_pib_2`. Looking at this general average, we can say that the GDP per capita of the municipalities that 
adopted the  Tarifa Zero is almost the double of the Brazilian municipalities that did not adopt it, however, when we observe this value by region and federative unit of the country, we verify that these values can vary widely. Below is a graph with all municipalities that adopted the policy together with the national median: 
...
banco_completo_5 %>% 
  mutate(media_nacional = median(Pib_Municipal_2018)) %>% 
  filter(tarifa_zero == "sim") %>% 
  mutate_if(is.character, str_to_title) %>%
  mutate(UF = case_when(UF == "Ce" ~ "CE", UF == "Mg" ~ "MG", UF == "Sp" ~"SP", 
                        UF == "Pr" ~ "PR", UF == "Rs" ~"RS", UF == "Go" ~"GO", 
...
In the table below we have the median per capita GDP by region in the two groups of municipalities. In it we can observe that the northeast and southeast regions, in fact, the median GDP per capita, in comparison, is much higher, however the cities in the center west and south have lower averages. 
...
pib_regiao_1 <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  group_by(REGIAO) %>% 
  summarize(media_pib_fftp = median(Pib_Municipal_2018))

pib_regiao_2 <- banco_completo_5 %>% 
  group_by(REGIAO) %>% 
  filter(tarifa_zero != "sim") %>% 
  summarize(media_pib_geral = median(Pib_Municipal_2018, na.rm = TRUE)) %>% 
  filter(REGIAO != " Norte") 
...
Here we can see the same data for the states with municipalities that adopted Tarifa Zero. Between the two group we have four states where the GPD is higher in the municipalities with Tarifa Zero: Ceará, Minas Gerais, Rio de Janeiro, São Paulo e Goiás. The other two, Paraná and Rio Grande do Sul, are the states with lower values, which follows the data by region of the country in regard to the South. 
...
pib_uf_1 <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  group_by(UF) %>% 
  summarize(media_1 = median(Pib_Municipal_2018))

pib_uf_2 <- banco_completo_5 %>% 
  filter(UF == "CE" | UF == "GO" | UF == "MG" | UF == "PR" | UF == "RJ" | UF == "RS" | UF == "SP") %>% 
  group_by(UF) %>% 
  filter(tarifa_zero != "sim") %>% 
  summarize(media_2 = median(Pib_Municipal_2018))
...
# média da taxa de ocupação  
  ...
  filter(tarifa_zero == "sim") %>% 
  summarize(media_1 = median(renda_percapta_media)) %>% 
  round(digits = 2) %>% 
  format_real()
...
The median income of the municipalities that adopted Tarifa Zero is `r renda_per_capta_fftp` reais, while for the municipalities that did not adopt the policy  is  
...
banco_completo_5 %>% 
  mutate(media_nacional = median(renda_percapta_media)) %>% 
  filter(tarifa_zero == "sim") %>% 
  mutate_if(is.character, str_to_title) %>%
  mutate(UF = case_when(UF == "Ce" ~ "CE", UF == "Mg" ~ "MG", UF == "Sp" ~"SP", 
                        UF == "Pr" ~ "PR", UF == "Rs" ~"RS", UF == "Go" ~"GO", 
...
media_ivs_1 <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(median(indice_ivs, na.rm = TRUE)) %>% 
  round(digits = 3) %>% 
  pull()
...
max_ivs_2 <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(max(indice_ivs, na.rm = TRUE)) %>% 
  round(digits = 3) %>% 
  pull()

min_ivs_urbano_1 <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(min(indice_ivs, na.rm = TRUE)) %>% 
  round(digits = 3) %>% 
  pull()
...
media_ivs_urbano_1 <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(median(ivs_infra_urbana, na.rm = TRUE)) %>% 
  round(digits = 3) %>% 
  pull()
...
Taking the IVS classification into account, the median of municipalities that adopted the Tarifa Zero was `r media_ivs_1`, 
which indicates very low social vulnerability, and the median in municipalities that did not adopt the policy was
`r media_ivs_2`, which also indicates a very low social vulnerability situation. The first group also has a maximum value of
...
media_idh_1 <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(median(idhm)) %>% round(digits = 2) %>% 
  pull()

media_idh_2 <- banco_completo_5 %>% 
  filter(tarifa_zero != "sim") %>% 
  summarize(median(idhm, na.rm = TRUE)) %>% round(digits = 2) %>% 
  pull()
...
media_idh_renda_1 <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(median(idhm_r)) %>% 
  round(digits = 2) %>% 
  pull()

media_idh_renda_2 <- banco_completo_5 %>% 
  filter(tarifa_zero != "sim") %>% 
  summarize(median(idhm_r, na.rm = TRUE)) %>% 
  round(digits = 2) %>% 
  pull()
...
media_idh_1 <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(median(idhm_e)) %>% 
  round(digits = 2) %>% 
  pull()

media_idh_2 <- banco_completo_5 %>% 
  filter(tarifa_zero != "sim") %>% 
  summarize(median(idhm_e, na.rm = TRUE)) %>% 
  round(digits = 2) %>% 
  pull()
...
media_idh_longevidade_1 <- banco_completo_5 %>% 
  filter(tarifa_zero == "sim") %>% 
  summarize(median(idhm_l)) %>% 
  round(digits = 2) %>% 
  pull()

media_idh_logenvidade_2 <- banco_completo_5 %>% 
  filter(tarifa_zero != "sim") %>% 
  summarize(median(idhm_l, na.rm = TRUE)) %>% 
  round(digits = 2) %>% 
  pull()
...
The median MHDI of the studied municipalities is `r media_idh_1`, which indicates "medium" MHDI, while the
median in Brazilian municipalities that have not adopted the policy is
`r media_idh_2`, which indicates MHDI "low". The median of the MHDI Income 
of the studied municipalities is of `r media_idh_renda_1`, that is, "high", while 
the average in Brazilian municipalities that have not adopted the policy is `r media_idh_renda_2`, medium.
...
values closer to 1, being  more to the left of the graphic. The median of the MHDI Education of municipalities with Tarifa Zero is `r media_idh_1`
while the median in Brazilian municipalities that have not adopted the policy
...
density graph. Finally, the median of the MHDI longevity of the studied municipalities is of
`r media_idh_longevidade_1` while the median in Brazilian municipalities
...
```{r Prosperidade Social , fig.align = 'center', out.width = "75%", dpi=300}
  ...
  ggtitle("") +
  scale_y_continuous(labels = scales::percent) + 
  scale_fill_manual(name = "", values = c("#954182", "#00688b"), labels = c("Municipalities with Tarifa Zero", "Municipalities without Tarifa Zero")) + 
