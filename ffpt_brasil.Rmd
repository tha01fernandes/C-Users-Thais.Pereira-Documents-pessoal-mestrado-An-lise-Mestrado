...
## 1. Base de Dados com Informações coletadas a respeito dos municípios

Abaixo há uma tabela com informações a respeito dos municípios que adotaram a política de Tarifa Zero. O ano da implementação foi coletado da dissertação de mestrado de Marijke Vermander (2021),"Exploring Fare-Free Public Transport in Brasil: Rationales and characteristics of Tarifa Zero policies in small Brazilian municipalities". O PIB per capta são dados do IBGE 2018, a taxa de urbanização utiliza dados da estimativa da população do IBGE de 2019, a densidade demográfica foi produzida com base em dados dessa mesma estimativa, no entando usando a área total do IBGE de 2020. Os dados de índice de vulnerabilidade social (IVS) são do IPEA e foram produdizos com base em dados do censo de 2010.  Todos os outros dados são provenientes do Índice de Desenvolvimento Humano dos municípios, coletados a partir do Atlas do Desenvolvimento Humano e são dados de 2010. Foi utilizado o pacote AbjData (https://abjur.github.io/abjData/) que contém esses dados sitematizados para todos os municípios brasileiros.   
...
# tabela com reactable: 
  ...
  reactable(
      ...
      taxa_pop_vulneravel_retorna_diariamente_trabalho = colDef("% da população ocupada vulnerável à pobreza que retorna diariamente do trabalho" , 
...
visualizar que mais da metade deles estão localizados no
...
# Tabela com a Região dos Municípios
  ...
  select(- n) %>% 
  kable(digits=1, col.names=c("Região", "Proporção de Municípios")) %>%
  kable_classic_2(full_width = F, html_font = "Cambria")

```

Dos 27 estados brasileiros, sete possuem algum município que implementou
FFPT. Minas Gerais é o estado com mais municípios no Brasil mas o
segundo com mais cidades com FFPT, e São Paulo é o segundo com mais
...
# Tabela com a UF dos municípios 
  ...
  mutate(pct = paste0(round(pct * 100, 0),"%")) %>% 
  select(- n) %>% 
  kable(digits=1, col.names=c("UF", "Proporção de Municípios")) %>%
  kable_classic_2(full_width = F, html_font = "Cambria")
```

## 3. Classe Populacional 

Na tabela abaixo podemos observar a proporção dos municípios em cada classe populacional, tanto dos municípios com Tarifa Zero, quanto dos que não adotaram a política. É possivel observar que que a maior parte dos municípios, 37% deles, tem população entre 20 e 50 mil habitantes enquanto 30% tem população entre 10 e 20 mil habitantes, ou seja, a maior parte dos municípios, 67% tem população entre 10 e 50 mil habitantes o que pode ser considerado de "pequeno porte", entre os municípios que não adotaram a politica, 44% deles percentem a essa classe populacional. É interessante notar que não há municípios com população até 5 mil habitantes e nem maior que 500 mil habitantes que tenha adotado a Tarifa Zero. 
...
No gráfico abaixo podemos ver a população estimada de 2019 do IBGE para todas as cidades que adotaram a Tarifa Zero, juntamente com a média nacional (linha vertical). A média populacional dos municípios com a Tarifa Zero é `r media_pop_ffpt` mil habitantes, sendo que há quantidades bem discrepantes, com um máximo de `r max_pop_ffpt`
como é o caso de Caucaia no Ceará, e um mínimo de `r min_pop_ffpt` habitantes em Pedro Osório no Rio Grande do Sul (IBGE, 2019). Nos municípios que não adotaram a política a média populacional é `r media_pop_geral`, mas também com valores muito discrepantes, chegando a um máximo de `r max_pop_geral` mil habitantes, como é o caso da cidade de São Paulo. No gráfico abaixo é possível comparar os municípios com a Tarifa Zero entre si: 
...
## 4. Ano de Implementação da Política de FFPT

A partir do gráfico abaixo podemos verificar que 70% dos municípios que adotaram FFPT o fizeram depois de 2010,
...
(MG)em 1997. Assim como a quatro municípios que adotaram em caracter permanente a política em 2021, são eles: 
Caucaia no Ceará, Caeté e Cláudio em Minhas Gerais e Formosa em Goiania. 
...
que adotaram FFPT e os que não adotaram. A taxa média do primeiro grupo é
`r media_urbanizacao_fftp`% enquanto do segundo é
`r media_urbanizacao_geral`%. No primeiro grupo, dois municípios aparecem com valores que se
...
política, até porque estão em muita maior quantidade, mas o interessante é observar que os municípios com Tarifa Zero 
estão mais a direita do gráfico, ou seja, trata-se de cidades bem urbanizadas em sua maioria. 
...
A densidade demográfica brasileira varia bastante, as capitais e grande metrópolis possuem uma densidade demográfica muito diferente das pequenas cidades mais ao interior do país. Os municípios que adotaram política de Tarifa Zero são em geral cidades de "pequeno porte", e por isso com baixíssima densidade demográfica, no gráfico abaixo podemos observar que apenas Vargem Grande Paulista em São Paulo chega a 1 habitante por km². A média nacional é 0,5 habitante por km², mas como dissemos antes, esse valor pode variar muito e por isso é bem pouco exato. Talvez ele seja muito pequeno pois no Brasil as cidades são em sua maioria de pequeno a médio porte mas no entanto maior parte da população está concentrada nas capitais e grandes metrópolis. 
...
No gráfico de densidade abaixo estamos mostrando apenas os municípios brasileiros com densidade demográfica menor que 0.4 habitante por km², ou seja, 5.189 municípios, para que o gráfico não ficasse com valores muito discrepantes e por conta disso, de difícil visualização. Nele podemos observar a concentração dos municípios com Tarifa Zero mais à direita do gráfico, o que indica que há um padrão de menor densidade demográfica nesse grupo em relação a todos os outros. 
...
FFPT apenas 18 têm a obrigação de desenvolver plano diretor. Nessa
...
Gini e L de Theil - L são dois índices de desigualdade bastante utilizados, ambos medem o
grau de concentração de renda em determinado grupo e variam de 0 a 1,
sendo 0 uma situação de completa igualdade, e 1 uma situação de completa
desigualdade, além disso ambos estão utilizando dados provenientes do censo 2010. 
No gráfico abaixo podemos observar a distribuição dos valores entre os municípios que adotaram a Tarifa Zero. 
A média geral do índice de Gini nos municípios estudados é
de `r media_indice_gini_tf`, enquanto dos municípios sem Tarifa Zero é
`r media_indice_gini_geral`. Há apenas uma pequena diferença nas médias.E o mesmo ocorre para o 
índice de Theil - L, em que a média no primeiro grupo é de `r `media_indice_theil_tf`, enquanto no segundo grupo é
de `r media_indice_theil_geral `, mas do mesmo modo os municípios com Tarifa Zero apresentam valores que indicam menor desigualdade
social. Abaixo podemos observar o gráfico da distribuição dos valores dos dois índices para todos
os municípios com Tarifa Zero: 
...
No gráfico de densidade abaixo é possível observar que há
algumas diferenças na distribuição dos valores mas elas são bem pequenas, a curva referente às
cidades com FFPT está mais à esquerda enquanto a dos outros municípios
está mais à direita.
...
banco_completo_5 %>% 
  ...
  mutate(name = case_when(
    name == "indice_gini" ~ "Índice de Gini",
    name == "indice_theil" ~ "Índice de Theil - L")) %>% 
  ggplot() +
  ...
  xlab("") +
  ylab("") +
  scale_fill_manual(name = "Tem Política de FFPT?", values = c("#00688b", "#954182"), labels = c("Não", "Sim"))+
  theme(plot.title = element_text(family = "serif", size = 14, face = "bold", hjust = 0.5, colour = "black"),
        strip.background = element_rect(colour = "white", fill = "#b9a6b3"),
...
O gráfico abaixo mostra a porcentagem de pobres em cada município estudado e a linha vertical representa a média nacional. 
...
do Brasil, e foo produzido a partir de dados do censo de 2010. Há uma diferença considerável entre as médias encontradas no
...
quanto a curva das cidades que adotaram FFPT está mais à esquerda do
gráfico.
...
Essa métrica de deslocamento e pobreza no Brasil é um dado poduzido a partir do censo de 2010. 
A média da porcentagem da população ocupada vulnerável à pobreza que retorna diariamente do trabalho nos 
municípios com Tarifa Zero é `r media_desloc2_fftp` enquanto nos outros municípios é de `r media_desloc2_geral`, ou seja, um pouco mais alto
mas ainda uma diferença muito pequena. Esses valores chegam a máximo de `r max_urbanizacao2_fftp` nos municípios com Tarifa Zero, enquanto 
`r max_urbanizacao2_geral` nos outros municiípios. 
A média da porcentagem de pessoas vulneráveis à pobreza que gastam mais de uma hora até o trabalho, por sua vez, nos municípios
que adotaram o FFPT é menos de 1%, ou mais precisamente
`r media_desloc_fftp`%, enquanto a média nos outros municípios é ligeiramente maior,
`r media_desloc_geral`%. Ou seja, a diferença é realmente bem pequena.
No primeiro essa porcetagem  chega a um valor máximo de
`r max_urbanizacao_fftp`% no município de Caucaia no estado do Ceará, enquanto nos outros municípios o máximo de  
`r max_urbanizacao_geral`%. 
...
## 11. PIB per capta dos municípios
...
```{r grafico densidade PIB, fig.align = 'center', out.width = "75%", dpi=300}
...
## 13. Taxa de Atividade 18 anos ou mais
...
## 15. Índice de Vulnerabilidade Social (IVS)
...
## 16. Índice de Desenvolvimento Humano (IDHM)
...
## 17. Motivação dos governos



## 18. Movimentos Sociais e Tarifa Zero
