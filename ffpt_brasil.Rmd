...
Tarifa Zero only 18 have an obligation to develop master plan. In this
...
urban, not considering the cities that did not respond. 
...
These are the six Municipalities with the Tarifa Zero Policy, which have
...
# tabela 
  ...
  tbl %>% select(name, Alta.x, Média.x, Baixa.x, `Muito Baixa.x`,  Alta.y,  Média.y, Baixa.y, `Muito Baixa.y`) %>% 
      ...
      Alta.x = colDef("High", minWidth = 70),
      Baixa.x = colDef("Low", minWidth = 70),
      Média.x = colDef("Medium", minWidth = 70), 
      `Muito Baixa.x` = colDef("Very Low", minWidth = 70),
      Alta.y = colDef("High", minWidth = 70),
      Baixa.y = colDef("Low", minWidth = 70),
      Média.y = colDef("Medium", minWidth = 70), 
      `Muito Baixa.y` = colDef("Very Low", minWidth = 70)), 
    columnGroups = list(
    colGroup(name = "Municipalities with Tarifa Zero", columns = c("Alta.x", "Baixa.x", "Média.x", "Muito Baixa.x")),
    colGroup(name = "Municipalities without Tarifa Zero", columns = c("Alta.y", "Baixa.y", "Média.y", "Muito Baixa.y"))))
...
# tabela 
  ...
  tbl_2 %>% 
      ...
      align = "center"),
      columns = list(
      name = colDef("Municipal Human development Index", minWidth = 140),
      Alto.x = colDef("High", minWidth = 70),
      Baixo.x = colDef("Low", minWidth = 70),
      Médio.x = colDef("Medium", minWidth = 70), 
      `Muito Alto.x` = colDef("Very High", minWidth = 70),
      Alto.y = colDef("High", minWidth = 70),
      Baixo.y = colDef("Low", minWidth = 70),
      Médio.y = colDef("Medium", minWidth = 70), 
      `Muito Alto.y` = colDef("Very High", minWidth = 70),
      `Muito Baixo` = colDef("Very Low", minWidth = 70)), 
    columnGroups = list(
    colGroup(name = "Municipalities with Tarifa Zero", columns = c("Alto.x", "Baixo.x", "Médio.x", "Muito Alto.x")),
    colGroup(name = "Municipalities without Tarifa Zero", columns = c("Alto.y", "Baixo.y", "Médio.y", "Muito Baixo","Muito Alto.y"))))
...
```{r mapa mov. sociais plot, fig.align = 'center', out.width = "150%", dpi=300}
...
# gráfico de mapa colocando os municípios dentro de seus respectivos estados: 
  ...
  geom_sf(data=muni_geobr_4, aes(fill = "Cities with Tarifa Zero's policy"), shape = 21,
          color = "white",
          size = 3,
          show.legend = T, 
          alpha = 0.8) +
