---
title: Análise Exploratória dos municípios que adotaram política de isenção de
...
**1. Apresentação do objeto de estudo, recorte empírico e municípios
estudados**

O objeto de estudo da pesquisa a qual esse relatório se refere tem como
recorte empírico o estudo de 27 cidades brasileiras que adotaram a
isenção integral da tarifa do transporte público. Para a seleção dos
...
# tabela com reactable: 
  ...
  select(UF, nome_muni,ano_implementacao, classe_pop, 
         ...
         taxa_atividade_18_anos_ou_mais, taxa_urbanizacao, ocup_desl, 
         indice_gini, indice_theil, idhm, idhm_e, idhm_r, idhm_l, indice_ivs, ivs_infra_urbana, ivs_capital_humano, ivs_renda_trabalho) %>% 
  ...
  reactable(
      ...
      prosperidade_social = colDef("Prosperidade Social", minWidth = 150),
      ...
      idhm = colDef("IDHM" ,  minWidth = 100),
      idhm_e = colDef("IDHM Educação" ,  minWidth = 100),
      idhm_r = colDef("IDHM Renda" ,  minWidth = 100), 
      idhm_l = colDef("IDHM Longevidade" , minWidth = 120),
      indice_ivs = colDef("IVS" , minWidth = 100), 
      ivs_infra_urbana = colDef("IVS Infraestrutura Urbana" , minWidth = 150), 
      ivs_renda_trabalho = colDef("IVS Renda e Trabalho" , minWidth = 150), 
      ivs_capital_humano = colDef("IVS Capital Humano" , minWidth = 150)), 
      outlined = FALSE, showPageInfo = TRUE, showPageSizeOptions = TRUE, 
      pageSizeOptions = c(5, 10, 20,30), defaultPageSize = 5, bordered = FALSE, striped = FALSE, highlight = TRUE)
...
# Gráfico ano de implementação 
  ...
  geom_col(width = 0.8, fill = "paleturquoise3",colour="white") +
  xlab("") +
  ylab("") +
...
# Gráfico taxa de urbanização boxplot
...
banco_completo_5 %>% 
  ...
  ggplot() +
  geom_boxplot(aes(x=tarifa_zero, y=taxa_urbanizacao),fill = "palevioletred", alpha=0.7)+
  ...
  theme(plot.title = element_text(family = "serif", size = 14, face = "bold", hjust = 0.5, colour = "black"),
    text = element_text(family = "serif"),
    axis.text.x=element_text(family = "serif", size = 12, colour = "black", face = "bold"),
    axis.text.y=element_text(size = 12, colour = "black"),
    legend.title = element_blank())

```

O gráfico de boxplot ilustra bem as médias da taxa de urbanização
enquanto o grafico de densidade, logo abaixo, ilustra melhor a
...
banco_completo_5  %>% 
  ...
  xlab("") +
  ylab("") +
  scale_fill_manual(name = "Tem Política de FFTP?", values = c("palevioletred", "paleturquoise3"), labels = c("Não", "Sim"))+
  theme(plot.title = element_text(family = "serif", size = 14, face = "bold", hjust = 0.5, colour = "black"),
    text = element_text(family = "serif"),
...
```{r Gráfico GINI e Theil, echo=FALSE, fig.align = 'center', out.width = "55%"}
...
```{r Gráfico Porcentagem de Pobres, echo=FALSE, fig.align='center', out.width= "50%"}
...
banco_completo_5 %>% 
  ...
  theme_minimal()+
  labs(fill = "Tem Política de FFTP?",
       title = "Porcentagem de Pobres no Município") +
  xlab("") +
  ylab("") +
  scale_fill_manual(name = "Tem Política de FFTP?", values = c("palevioletred", "paleturquoise3"), labels = c("Não", "Sim"))+
  theme(plot.title = element_text(family = "serif", size = 14, face = "bold", hjust = 0.5, colour = "black"),
...
```{r Deslocamento e Pobreza, echo=FALSE, fig.align = 'center', out.width = "50%"}
...
banco_completo_5 %>% 
  ...
  ggplot() +
  geom_boxplot(aes(x=tarifa_zero, y=ocup_desl),fill = "palevioletred", alpha=0.7)+
  scale_y_continuous(labels = scales::percent)+
  xlab("") +
  ylab("") +
  labs(title = "Deslocamento Urbano e Pobreza", 
       subtitle = "% de vulneráveis à pobreza que gastam mais de uma hora até o trabalho") +
  theme_minimal() + 
  theme(plot.title = element_text(family = "serif", size = 14, face = "bold", hjust = 0.5, colour = "black"),
    text = element_text(family = "serif"),
    axis.text.x=element_text(family = "serif", size = 11, colour = "black", face = "bold"),
    axis.text.y=element_text(size = 12, colour = "black"),
    plot.subtitle = element_text(family = "roboto", size = 11, color = "#404040", hjust = 0.5),
    legend.title = element_blank())


```

**10. PIB per capta dos municípios**
...
**11. Taxa de Atividade 18 anos ou mais**
...
```{r Taxa de Atividade 18 anos ou mais, fig.align = 'center', out.width = "50%"}
...
banco_completo_5 %>%   
  ...
  theme_minimal()+
  labs(fill = "Tem Política de FFTP?",
       title = "Taxa de Atividade (18 anos ou mais)") +
  xlab("") +
  ylab("") +
  scale_fill_manual(name = "Tem Política de FFTP?", values = c("palevioletred", "paleturquoise3"), labels = c("Não", "Sim"))+
  theme(plot.title = element_text(family = "serif", size = 14, face = "bold", hjust = 0.5, colour = "black"),
...
```{r Renda percapta média, fig.align = 'center', out.width = "50%"}
...
banco_completo_5 %>% 
  ...
  labs(title = "Renda Média Per Capta") +
  xlab("") +
  ylab("") +
  scale_fill_manual(name = "Tem Política de FFTP?", values = c("palevioletred", "paleturquoise3"), labels = c("Não", "Sim"))+
  theme_minimal()+
...
**13. Índice de Vulnerabilidade Social IVS**
...
Levando essa classificação em conta, a média dos municípios que
adotaram a FFTP ficou em `r media_ivs_1` o que indica baixa
vulneravilidade social. A media nos municípios que não adotaram a
política ficou em `r media_ivs_2`, o que indica uma situação de média
vulnerabilidade social. O primeiro grupo também apresenta um valor
máximo de `r max_ivs_2` referente a cidade de Caucaia no Ceará,
representando alta vulnerabilidade social, e no outro extreno o
valor de `r min_ivs_urbano_1` na cidade de Holambra em São Paulo,
correspondendo a muita baixa vulnerabilidade social. No gráfico
abaixo podemos observar que os municípios com FFTP estão mais à
esquerda do gráfico em relação a todos os outros municípios
brasileiros o que indica que eles estão, em geral, em situção de
menor vulnerabilidade social, sendo o indicador de infraestrutura
urbana o único com bem pouca diferença entre os dois grupos.
Comparando as médias desse último indicador temos que os municípios
com FFTP apresentam o valor de `r media_ivs_urbano_1`, enquanto nos
municípios que não adotaram é `r media_ivs_urbano_2`, há bem pouca
diferença, no entanto sugere que o primeiro grupo de cidades tem um
infraestrutura urbana melhor que o segundo grupo.

```{r grafico ivs, fig.align = 'center', out.width = "60%"}
...
**14. Índice de Desenvolvimento Humano IDHM**
...
próximo de 1, maior o desenvolvimento humano."(PNUD,2021) O IDHM médio
dos municípios estudados é de `r media_idh_1` enquanto a média nos
municípios brasileiros que não adotaram a política é `r media_idh_2`.
Olhando para a distribuição dos valores para os dois grupos de
municípios podemos perceber que os municípios que adotaram a política
possuem valores de IDHM ligeiramente maiores.

```{r Grafico IDHM, fig.align = 'center', out.width = "60%"}
...
`r media_idh_renda_1` enquanto a média nos municípios brasileiros que
não adotaram a política é `r media_idh_renda_2`, valores bem próximos.
No entanto, quando olhamos para a distribuição dos valores no gráfico de
...
é `r media_idh_2`. Assim, como no gráfico de densidade do IDHM geral, os
municípios com FFTP apresentam valores mais próximos de 1, à esquerda do
gráfico, em relação a distribuição dos valores referentes aos municípios
que não adotaram a políica.
...
que não adotaram a política é `r media_idh_logenvidade_2`.Aqui há também
uma diferença entre os municípios com o FFTP e os sem a política, o
primeiro grupo apresenta valores mais próximos de 1, assim como nos
outros indicadores, mas também um concentração grande de municípios em
torno da média.

A respeito dos indicadores de IDHM: renda, educação e longevidade,
podemos dizer que, em termos de longevidade e educação, os municípios
...
relação a renda, as médias apresentam valores praticamente semelhantes.

**15. Movimentos Sociais e Tarifa Zero**
...
```{r mapa,fig.align = 'center', out.width = "50%"}
...
# gráfico de mapa colocando os municípios dentro de seus respectivos estados: 
  ...
  geom_sf(data=muni_geobr_4, color = "lightseagreen", size = 1.5) +
  geom_sf(data= muni_geobr_5, color = "blueviolet", size = 1.5) +
  geom_sf(data= muni_geobr_6, color = "darkorange", size = 1.5) + 
  theme_void()+
  scale_color_manual(name = "Legenda", values = c("palevioletred", "blueviolet"), labels = c("Cidades com Tarifa Zero", "Presença do Movimento Passe Livre"))+
   labs(fill = "",
        title = "Movimento Passe Livre, Movimento Tarifa Zero e Cidades que Implementaram Tarifa Zero", 
        subtitle = "Os pontos roxos representam o movimento Passe Livre, os laranjas representam o movimento Tarifa Zero,\n e os pontos em verde as cidades com política de tarifa zero")+
  theme(plot.title = element_text(family = "serif", size = 12, face = "bold", hjust = 0.5, colour = "black"),
       plot.subtitle = element_text(family = "roboto", size = 10, color = "#404040", hjust = 0.5))
