...
# tabela com reactable: 
  ...
  select(UF, nome_muni,ano_implementacao, classe_pop, 
         ...
         taxa_atividade_18_anos_ou_mais, taxa_urbanizacao, ocup_desl, taxa_pop_vulneravel_retorna_diariamente_trabalho,
...
```{r Taxa de Urbanização, fig.align = 'center', out.width = "60%"}
...
banco_completo_5 %>% 
  ...
  mutate(UF = case_when(UF == "Ce" ~ "CE", UF == "Mg" ~ "MG", UF == "Sp" ~"SP", 
                      ...
                      UF == "Rj" ~ "RJ", TRUE ~ UF)) %>% 
  ggplot() + 
  geom_col(aes(x = reorder(nome_muni, taxa_urbanizacao), y = taxa_urbanizacao, fill = UF), width = 0.65,colour="white") +
   geom_errorbar(mapping = aes(x = nome_muni, y = media_nacional, ymin = media_nacional, ymax = media_nacional), size = 0.5) +
  xlab("") +
  ylab("") +
  ggtitle("Taxa de Urbanização dos Municípios com Tarifa Zero") +
  scale_y_continuous(labels = scales::percent)+
scale_fill_brewer(name = "UF", palette = "Set3")+
#  geom_label(aes(x = nome_muni, y = taxa_urbanizacao, label = taxa_urbanizacao), 
#           position = position_stack(vjust = 0.8)) +
  theme_minimal() + 
...
```{r Taxa de urbanização dos municípios boxplot, echo=FALSE, fig.align = 'center', out.width = "50%"}
...
```{r Taxa de Urbanização densidade, echo=FALSE, fig.align = 'center', out.width = "50%"}
...
```{r Porcentagem de pobres, fig.align = 'center', out.width = "60%"}
...
banco_completo_5 %>% 
  ...
  mutate(UF = case_when(UF == "Ce" ~ "CE", UF == "Mg" ~ "MG", UF == "Sp" ~"SP", 
                      ...
                      UF == "Rj" ~ "RJ", TRUE ~ UF)) %>% 
  ggplot() + 
  geom_col(aes(x = reorder(nome_muni, pct_pobres), y = pct_pobres, fill = UF), width = 0.65,colour="white") +
   geom_errorbar(mapping = aes(x = nome_muni, y = media_nacional, ymin = media_nacional, ymax = media_nacional), size = 0.5) +
  xlab("") +
  ylab("") +
  ggtitle( "Porcentagem de pobres dos Municípios com Tarifa Zero") +
...
```{r Porcentagem de Pobres densidade, echo=FALSE, fig.align='center', out.width= "50%"}
...
```{r media deslocamento e pobreza médias}
...
```{r Delocamento e Pobreza 1, fig.align = 'center', out.width = "60%"}

banco_completo_5 %>% 
  mutate(media_nacional = mean(taxa_pop_vulneravel_retorna_diariamente_trabalho)) %>% 
  filter(tarifa_zero == "sim") %>% 
  mutate_if(is.character, str_to_title) %>%
  mutate(UF = case_when(UF == "Ce" ~ "CE", UF == "Mg" ~ "MG", UF == "Sp" ~"SP", 
                        UF == "Pr" ~ "PR", UF == "Rs" ~"RS", UF == "Go" ~"GO", 
                      UF == "Rj" ~ "RJ", TRUE ~ UF)) %>% 
  ggplot() + 
  geom_col(aes(x = reorder(nome_muni, taxa_pop_vulneravel_retorna_diariamente_trabalho), y = taxa_pop_vulneravel_retorna_diariamente_trabalho, fill = UF), width = 0.65,colour="white") +
   geom_errorbar(mapping = aes(x = nome_muni, y = media_nacional, ymin = media_nacional, ymax = media_nacional), size = 0.5) +
  xlab("") +
  ylab("") +
  labs(title = "Deslocamento Urbano e Pobreza", 
       subtitle = "% da população ocupada vulnerável à pobreza que retorna diariamente do trabalho") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_fill_brewer(name = "UF", palette = "Set3")+
  theme_minimal() + 
    theme(plot.title = element_text(family = "serif", size = 12, face = "bold", hjust = 0.5, colour = "black"),
...
```{r Delocamento e Pobreza 2, fig.align = 'center', out.width = "60%"}

banco_completo_5 %>% 
  mutate(media_nacional = mean(ocup_desl/100)) %>% 
  ...
  mutate(UF = case_when(UF == "Ce" ~ "CE", UF == "Mg" ~ "MG", UF == "Sp" ~"SP", 
                      ...
                      UF == "Rj" ~ "RJ", TRUE ~ UF)) %>% 
  ggplot() + 
  geom_col(aes(x = reorder(nome_muni, ocup_desl), y = ocup_desl, fill = UF), width = 0.65,colour="white") +
   geom_errorbar(mapping = aes(x = nome_muni, y = media_nacional, ymin = media_nacional, ymax = media_nacional), size = 0.5) +
  xlab("") +
  ylab("") +
  labs(title = "Deslocamento Urbano e Pobreza", 
...
```{r Deslocamento e Pobreza boxplot, echo=FALSE, fig.align = 'center', out.width = "50%"}
...
```{r PIB, fig.align = 'center', out.width = "65%"}
...
```{r Prosperidade Social , fig.align = 'center', out.width = "50%"}
  ...
  ggplot(aes(x = reorder(prosperidade_social, - pct_prosperidade), y = pct_prosperidade)) + 
  geom_col(width = 0.8, fill = "paleturquoise3",colour="white") +
  xlab("") +
  ylab("") +
  ggtitle("Prosperidade Social") +
  scale_y_continuous(labels = scales::percent) + 
  geom_label(aes(x = prosperidade_social, y = pct_prosperidade, label = scales::percent(pct_prosperidade, accuracy = .1)), 
             position = position_identity(),family = "serif", face = "bold") +
  theme_minimal() + 
    theme(plot.title = element_text(family = "serif", size = 16, face = "bold", hjust = 0.5, colour = "black"),
    text = element_text(family = "serif"),
    axis.text.x=element_text(size = 13, family = "serif"),
...
```{r Taxa de Atividade 18 anos ou mais, fig.align = 'center', out.width = "60%"}
...
banco_completo_5 %>% 
  ...
  xlab("") +
  ylab("") +
  labs(title = "Taxa de Atividade (18 anos ou mais)") + 
  scale_y_continuous(labels = scales::percent) +
  scale_fill_brewer(name = "UF", palette = "Set3")+
  theme_minimal() + 
    theme(plot.title = element_text(family = "serif", size = 12, face = "bold", hjust = 0.5, colour = "black"),
...
```{r Taxa de Atividade 18 anos ou mais densidade, fig.align = 'center', out.width = "50%"}
...
```{r Renda per capta, fig.align = 'center', out.width = "60%"}
...
```{r Renda percapta média densidade, fig.align = 'center', out.width = "50%"}
...
# tabela 
  ...
  tbl_2 %>% 
    ...
    columnGroups = list(
    colGroup(name = "Municípios com FFPT", columns = c("Alto.x", "Baixo.x", "Médio.x", "Muito Alto.x")),
    colGroup(name = "Municípios sem FFPT", columns = c("Alto.y", "Baixo.y", "Médio.y", "Muito Baixo","Muito Alto.y"))))
...
**15. Motivação dos governos**



**16. Movimentos Sociais e Tarifa Zero**
...
```{r mapa,fig.align = 'center', out.width = "70%"}
...
# gráfico de mapa colocando os municípios dentro de seus respectivos estados: 
  ...
  geom_sf(data=muni_geobr_4, color = "lightseagreen", size = 2, aplha = 0.5) +
  geom_sf(data= muni_geobr_5, color = "blueviolet", size = 2, alpha = 0.5) +
  geom_sf(data= muni_geobr_6, color = "red", size = 2, alhpa = 0.5) + 
  theme_void()+
  scale_color_manual(name = "Legenda", values = c("palevioletred", "blueviolet"), labels = c("Cidades com Tarifa Zero", "Presença do Movimento Passe Livre"))+
   labs(fill = "",
        title = "Pontos roxos: movimento Passe Livre,\n Pontos vermelhos: movimento Tarifa Zero,\n  Pontos verdes: cidades com política de tarifa zero", 
        caption = "")+
